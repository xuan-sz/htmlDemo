...
;(function(factory) {
    ...
    EvoCalendar = (function() {
        ...
        function EvoCalendar(element, settings) {
            ...
            _.defaults = {
                ...
                calendarEvents: null,
                switchTitleDisplayDefault: false
            };
            ...
            _.initials = {
                ...
                dates: {
                    cn: {
                        ...
                        noEventForToday: "今天没有活动，所以休息一下！ :)",
                        noEventForThisDay: "今天没有活动，所以休息一下！ :)",
                        previousMonthText: "上一个月",
                        nextMonthText: "下一个月",
                        previousYearText: "上一年",
                        nextYearText: "下一年",
                        closeSidebarText: "关闭侧边栏",
                        closeEventListText: "关闭事件窗口"
                    },
                    hk: {
                        days: ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"],
                        daysShort: ["周日", "周一", "周二", "周三", "周四", "周五", "周六"],
                        ...
                        months: ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"],
                        monthsShort: ["一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二"],
                        noEventForToday: "今日沒有活動，所以休息下！ :)",
                        noEventForThisDay: "今日沒有活動，所以休息下！ :)",
                        previousMonthText: "上一个月",
                        nextMonthText: "下一个月",
                        previousYearText: "上一年",
                        nextYearText: "下一年",
                        closeSidebarText: "关闭側邊欄",
                        closeEventListText: "關閉事件窗口"
                    },
                    en: {
            ...
            _.switchMonth = $.proxy(_.switchMonth, _);
    ...
    EvoCalendar.prototype.initEventListener = function() {
        ...
        // set event listener for switch each month
        _.$elements.innerEl.find('[data-switch-month-val]')
        .off('click.evocalendar')
        .on('click.evocalendar', _.switchMonth);
    ...
    EvoCalendar.prototype.buildEventList = function() {
        ...
        var eventFooterEl = _.$elements.eventEl.find('.event-footer');
        // Clear event list item(s)
        if (eventListEl.children().length > 0) eventListEl.empty();
        if (eventFooterEl.children().length > 0) eventFooterEl.empty();
    ...
    EvoCalendar.prototype.addEventList = function(event_data) {
        ...
        markup += '<div class="event-icon"';
        if (event_data.color) {
            markup += 'style="background-color:'+event_data.color+'"'
        }
        markup +='><div class="event-bullet-'+event_data.type+'"';
        if (event_data.color) {
            markup += 'style="background-color:'+event_data.color+'"'
        }
        markup += '></div></div><div class="event-info" '
        if (event_data.color) {
            markup += 'style="background-color:'+hexToRgba(event_data.color, 0.2)+'"'
        }
        markup += '><p class="event-title">'+_.limitTitle(event_data.name);
        ...
        markup += '</div>';
        eventListEl.append(markup);
        if( event_data.footer ) eventFooterEl.append(`<p>${event_data.footer}</p>`);
    ...
    EvoCalendar.prototype.buildCalendar = function() {
        ...
        if(_.options.switchTitleDisplayDefault) {
            let titleEls = '<div class="calendar-header-title"> '+
                '<button class="switch-btn" role="button" data-switch-month-val="prev" title="'+_.initials.dates[_.options.language].previousMonthText+'">&lt;</button>' +
                '<label class="center-label">'+ title +'</label>' +
                '<button class="switch-btn" role="button" data-switch-month-val="next" title="'+_.initials.dates[_.options.language].nextMonthText+'">&gt;</button>' +
            '</div> ';
            _.$elements.innerEl.find('.calendar-table th').html(titleEls);
    ...
    EvoCalendar.prototype.addEventIndicator = function(event) {
        ...
        var event_bg_color = event.bgColor;
        ...
        if (event_date instanceof Array) {
            ...
            for (var i=0; i<active_date.length; i++) {
                ...
                setDayBackgroudColor(active_date[i])
            }
        } else {
            if (event.everyYear) {
            ...
            setDayBackgroudColor(event_date)
        }
    ...
    ****************/
    ...
    // v1.0.0 - Swith month
    EvoCalendar.prototype.switchMonth = function(event) {
        var _ = this;
        var el, monthVal;
        ...
        if (typeof event === 'string' || typeof event === 'number') {
            ...
            el = $(event.target).closest('[data-switch-month-val]');
            monthVal = $(el).data('switchMonthVal');
        }

        if(monthVal == "prev" && _.$active.month !== 0 ) {
            --_.$active.month;
        } else if (monthVal == "prev" && _.$active.month === 0 ) {
            --_.$active.year;
            _.$active.month = 11;
        } else if (monthVal == "next" && _.$active.month !== 11) {
            ++_.$active.month;
        } else if (monthVal == "next" && _.$active.month === 11) {
            ++_.$active.year;
            _.$active.month = 0;
